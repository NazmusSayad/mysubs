...
class ClaudeProvider {
  ...
  private loadAuth(configDir: string): AuthState {
    ...
    if (shared) found = this.readFromKeychain()
  ...
  private needsRefresh(oauth: Oauth): boolean {
  ...
  private async refreshAccessToken(state: AuthState): Promise<string> {
    ...
    try {
      ...
      process.stderr.write(
        `mysubs: could not persist refreshed claude credentials: ${this.errorMessage(error)}\n`
      )
    }
  ...
  private requestUsage(accessToken: string): Promise<Response> {
  ...
  private rateLimitMessage(response: Response): string {
  ...
  async fetchUsage(): Promise<AccountUsageResult> {
    ...
    if (this.needsRefresh(state.oauth)) {
      accessToken = await this.refreshAccessToken(state)
    }

    let response = await this.requestUsage(accessToken)
    if (response.status === 401 || response.status === 403) {
      accessToken = await this.refreshAccessToken(state)
      response = await this.requestUsage(accessToken)
      if (response.status === 401 || response.status === 403) {
        throw new Error('session expired, run `claude` to log in again')
      }
    }
    ...
    if (response.status === 429) {
      throw new Error(this.rateLimitMessage(response))
    }
    if (!response.ok) {
      throw new Error(
    ...
    const result = this.mapUsage(parsed.data)

    const plan = this.formatPlan(
  ...
  private resetTime(value: string | number | null | undefined): string | null {
  ...
  private windowResource(
    ...
    const resetsAt = this.resetTime(resets)
    if (resetsAt !== null) resource.resetsAt = resetsAt
    return resource
  }

  private assignWindow(
    ...
    usage[key] = this.windowResource(window.utilization, window.resets_at)
  }

  private assignScopedLimits(
    usage: Record<string, UsageResource>,
    limits: unknown[]
  ): void {
  ...
  private mapUsage(body: z.infer<typeof usageSchema>): AccountUsageResult {
    ...
    this.assignWindow(usage, 'weekly', body.seven_day)
    this.assignWindow(usage, 'opus', body.seven_day_opus)
    this.assignWindow(usage, 'sonnet', body.seven_day_sonnet)
    this.assignScopedLimits(usage, body.limits ?? [])
  ...
  private accountConfigPaths(configDir: string): string[] {
    const paths = [
  ...
  private accountLabel(configDir: string): string | null {
    for (const file of this.accountConfigPaths(configDir)) {
