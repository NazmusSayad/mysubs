...
function readAuthText(configDir: string): { text: string; source: AuthSource } {
...
function loadAuth(configDir: string): AuthState {
  const { text, source } = readAuthText(configDir)
...
async function refreshAccessToken(state: AuthState): Promise<string> {
  ...
  try {
    ...
    process.stderr.write(
      `mysubs: could not persist refreshed codex credentials: ${errorMessage(error)}\n`
    )
  }
...
function refreshErrorCode(body: unknown): string | null {
...
export async function fetchCodexUsage(
  ...
  let response = await fetchUsage(accessToken, state.auth.tokens?.account_id)
  if (response.status === 401 || response.status === 403) {
    accessToken = await refreshAccessToken(state)
    response = await fetchUsage(accessToken, state.auth.tokens?.account_id)
    if (response.status === 401 || response.status === 403) {
      throw new Error('session expired, run `codex` to log in again')
    }
  }
...
function windowResource(window: Window, usedPercent: number): UsageResource {
...
function creditBalance(
  ...
  return headerNumber(response, 'x-codex-credits-balance')
}
